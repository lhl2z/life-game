	...
	for(i = 0;i < M;i++)
	{
		for(j = 0;j < N;j++)
		{
			...
			if(0 == i)
			{
				...
				{
					count = nowMap[i][j-1]+nowMap[i+1][j-1]+nowMap[i][j+1]
			...
			else if(M-1 == i)
			{
				...
				// 左边中间的情况
				count = nowMap[i-1][j]+nowMap[i-1][j+1]+nowMap[i][j+1]
				...
				count = nowMap[i-1][j]+nowMap[i-1][j-1]+nowMap[i][j-1]
				        +nowMap[i+1][j-1]+nowMap[i+1][j]+nowMap[i-1][j+1]
						+nowMap[i][j+1]+nowMap[i+1][j+1];
